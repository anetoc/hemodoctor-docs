...
def test_pipeline_tma_critical():
    ...
    Expected:
        ...
        - Short-circuit (stop after first critical)
    ...
    assert result["top_syndromes"][0] == "S-TMA", "S-TMA should be first (critical)"
    ...
    # Short-circuit check
    assert len(result["top_syndromes"]) == 1, "Should stop after first critical syndrome"

    # Detail check
    syndromes_detail = result["syndromes_detail"]
    assert len(syndromes_detail) > 0, "Should have syndrome details"
    assert syndromes_detail[0]["id"] == "S-TMA"
    assert syndromes_detail[0]["criticality"] == "critical"
...
def test_pipeline_short_circuit():
    """
    Test short-circuit behavior (stop after first critical).

    Critical + Priority syndromes present:
        - S-TMA (critical) → should short-circuit
        - S-IDA (priority) → should NOT be detected

    Expected:
        - Only S-TMA detected
        - S-IDA NOT detected (short-circuit)
    """
    cbc = {
        # TMA (critical)
        "plt": 8,
    ...
    # Should detect TMA (critical)
    assert "S-TMA" in result["top_syndromes"]

    # Should stop after first critical (short-circuit)
    assert len(result["top_syndromes"]) == 1, "Should short-circuit after S-TMA"

    # S-IDA should NOT be detected (even though evidence present)
    assert "S-IDA" not in result["top_syndromes"], "S-IDA should not be detected (short-circuit)"
