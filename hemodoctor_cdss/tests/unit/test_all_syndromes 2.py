...
def test_critical_syndrome_short_circuit(yaml_parser):
    """
    Test Solution 2 behavior (collect ALL critical syndromes before short-circuit).

    BUG-018 FIX: Updated for Solution 2 (REQ-HD-034)
    - Multiple critical syndromes are allowed (co-occurrence)
    - All critical syndromes are evaluated before short-circuiting
    - Short-circuit only prevents evaluation of lower-priority syndromes
    """
    ...
    evidences_multiple_critical = [
        ...
        EvidenceResult(id="E-WBC-VERY-HIGH", status="present", strength="strong"),  # S-BLASTIC-SYNDROME (may trigger)
    ]
    ...
    assert len(syndromes) >= 1, \
        f"Expected at least 1 critical syndrome, got {len(syndromes)}"

    # All returned syndromes must be critical (short-circuit prevents priority)
    for syndrome in syndromes:
        assert syndrome.criticality == "critical", \
            f"Expected all syndromes to be critical, got {syndrome.criticality} for {syndrome.id}"

    # Verify specific critical syndromes are detected
    syndrome_ids = [s.id for s in syndromes]
    assert "S-NEUTROPENIA-GRAVE" in syndrome_ids, "S-NEUTROPENIA-GRAVE must be detected"
    assert "S-PLT-CRITICA" in syndrome_ids, "S-PLT-CRITICA must be detected"
...
def test_nested_logic_syndromes_work(yaml_parser):
    ...
    # BUG-019 FIX: Solution 2 allows multiple critical syndromes
    # Should detect BOTH S-BLASTIC-SYNDROME (via nested logic) and S-PLT-CRITICA (via E-PLT-CRIT-LOW)
    syndrome_ids = [s.id for s in syndromes2]
    assert "S-BLASTIC-SYNDROME" in syndrome_ids, \
        "S-BLASTIC-SYNDROME not detected (nested logic branch 2 failed)"
    assert all(s.criticality == "critical" for s in syndromes2), \
        "All detected syndromes must be critical (short-circuit after critical)"
